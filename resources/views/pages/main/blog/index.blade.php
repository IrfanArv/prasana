...
@section('content')
    <style>
        ...
        .blog-featured-overlay .blog-date {
            font-size: 12px;
            color: rgba(255, 255, 255, 0.7);
            margin-bottom: 6px;
        }
        ...
        .blog-card .blog-tag {
            display: inline-block;
            background: rgba(165, 134, 57, 0.12);
            color: #A58639;
            padding: 3px 10px;
            font-size: 10px;
            text-transform: uppercase;
        ...
        .blog-sidebar .search-form input:focus {
            border-color: #A58639;
            box-shadow: 0 0 0 3px rgba(165, 134, 57, 0.15);
        }
        ...
        .blog-sidebar .category-list li a:hover span {
            background: rgba(165, 134, 57, 0.15);
            color: #A58639;
        }
        ...
        .blog-sidebar .tag-cloud a:hover {
            ...
            box-shadow: 0 2px 8px rgba(165, 134, 57, 0.3);
        }
            ...
            <div class="blog-featured-grid">
                <div class="blog-featured-item">
                    <a href="{{ route('blog.detail', $featuredPosts[0]->slug) }}">
                        <img src="{{ $featuredPosts[0]->cover_image ? asset('/img/blog/' . $featuredPosts[0]->cover_image) : 'https://dummyimage.com/800x400/333/aaa&text=No+Cover' }}"
                    ...
                    <div class="blog-featured-overlay">
                        ...
                        <h3><a
                                href="{{ route('blog.detail', $featuredPosts[0]->slug) }}">{{ $featuredPosts[0]->title }}</a>
                        </h3>
                    </div>
                </div>
                <div class="blog-featured-item">
                    <a href="{{ route('blog.detail', $featuredPosts[1]->slug) }}">
                        <img src="{{ $featuredPosts[1]->cover_image ? asset('/img/blog/' . $featuredPosts[1]->cover_image) : 'https://dummyimage.com/800x400/333/aaa&text=No+Cover' }}"
                    ...
                    <div class="blog-featured-overlay">
                        ...
                        <h3><a
                                href="{{ route('blog.detail', $featuredPosts[1]->slug) }}">{{ $featuredPosts[1]->title }}</a>
                        </h3>
                    </div>
                </div>
            </div>
                            ...
                            <div class="blog-card">
                                <div class="card-img">
                                    <a href="{{ route('blog.detail', $post->slug) }}">
                                        <img src="{{ $post->cover_image ? asset('/img/blog/' . $post->cover_image) : 'https://dummyimage.com/400x220/eee/999&text=No+Cover' }}"
                                            alt="{{ $post->title }}">
                                    </a>
                                </div>
                                <div class="card-body">
                                    @foreach ($post->categories->take(2) as $cat)
                                        <a href="{{ route('blog.category', $cat->slug) }}"
                                            class="blog-tag">{{ $cat->name }}</a>
                                    @endforeach
                                    <h5 class="card-title"><a
                                            href="{{ route('blog.detail', $post->slug) }}">{{ $post->title }}</a></h5>
            ...
            <div class="col-md-8">
                            ...
                            @if ($key === 0)
                                <div class="col-12 mb-4">
                                    <div class="blog-card">
                                        <div class="card-img">
                                            <a href="{{ route('blog.detail', $post->slug) }}">
                                                <img src="{{ $post->cover_image ? asset('/img/blog/' . $post->cover_image) : 'https://dummyimage.com/800x350/eee/999&text=No+Cover' }}"
                                                    alt="{{ $post->title }}" style="height: 350px;">
                                            </a>
                                        </div>
                                        <div class="card-body">
                                            @foreach ($post->categories->take(2) as $cat)
                ...
                @else
                    ...
                    <div class="empty-state">
                        ...
                        <h4>No posts yet</h4>
                        <p>{{ $emptyMessage ?? 'We\'re working on some great content. Check back soon for the latest stories and updates!' }}
                        </p>
                        @if (isset($pageTitle))
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            if (burger1) {
                ...
                burger1.classList.remove('color-nav-white');
            }
            if (burger2) {
                burger2.classList.add('color-nav-dark');
                burger2.classList.remove('color-nav-white');
            }
            if (burger3) {
                burger3.classList.add('color-nav-dark');
                burger3.classList.remove('color-nav-white');
            }
